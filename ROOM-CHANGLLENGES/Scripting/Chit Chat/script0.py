...
key = re.search(r'key:(\S+)', data_str).group(1).encode('utf-8')  # Key dạng bytes
iv = re.search(r'iv:(\S+)', data_str).group(1).encode('utf-8')    # IV dạng bytes
...
# Nhận dữ liệu tag từ server
sock.sendto(b'final', server_address)
tag, _ = sock.recvfrom(1024)
print(f"Received tag (in bytes): {tag}")
print("-----------")

# Hàm giải mã AES-GCM
def decrypt(key, iv, ciphertext, tag):
    try:
        # Tạo AES cipher với chế độ GCM
...
# Giải mã dữ liệu
decrypted_data = decrypt(key, iv, enc_data, tag)
if decrypted_data:
