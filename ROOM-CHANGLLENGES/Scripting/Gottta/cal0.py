...
def parse_operation_response(response, number):                                                                         
    ...
    if operation == 'add':                                 
        ...
        if value != 0:                                                                                                
            number /= value  # Sử dụng phép chia thực                                                                
        else:                                              
            print("Division by zero. Skipping divide")                                  
            return None, None                                                                                         
    return round(number, 3), next_port                                                             
                                                                                                                      
def main():                                                                                                           
    host = '10.10.216.50'                                  
    port = 1337                                                                                                        
    number = 0.0  # Chuyển đổi số thành kiểu float                                            
    retries = 50   # Đặt số lần thử lại
    delay = 0.01  # Khoảng thời gian chờ giữa mỗi lần thử lại
    ...
    # Làm tròn kết quả cuối cùng đến hai chữ số thập phân
    number = round(number, 3)
    print(f"Final number (rounded to 2 decimal places): {number}")
